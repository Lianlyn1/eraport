     ...
     <div class="col s12 m12 l12">
       <div class="card-panel">
         ...
         <table>
           ...
           <?php
            ...
            if (isset($_POST['kelas'])) {
              $kelas = $_POST['kelas'];
              $no = 1;
              $lihat = mysqli_query($connect, "SELECT siswa.kode_siswa, siswa.nis, siswa.nama_siswa, nilai.semester, pelajaran.nama_pelajaran, nilai.nilai_tugas, nilai.nilai_tugas2, nilai.nilai_tugas3, nilai.nilai_uts, nilai.nilai_uas, nilai.keterangan, datakelas.jurusan, guru.nama_guru FROM siswa, nilai, pelajaran, datakelas, kelas, guru WHERE siswa.kode_siswa=nilai.kode_siswa AND nilai.kode_pelajaran=pelajaran.kode_pelajaran AND datakelas.kode_siswa=siswa.kode_siswa AND datakelas.kode_kelas=kelas.kode_kelas AND kelas.kode_kelas=nilai.kode_kelas AND kelas.kode_guru=guru.kode_guru AND kelas.kelas='$kelas' AND siswa.kode_siswa='$siswa'") or die(mysqli_error($connect));
              $ambilguru = mysqli_fetch_array($lihat);
              if (mysqli_num_rows($lihat) > 0) {
                $qry = mysqli_query($connect, "SELECT tahun_ajar, kelas, nama_kelas,  semester FROM nilai, kelas, siswa WHERE nilai.kode_siswa=siswa.kode_siswa AND kelas.kode_kelas=nilai.kode_kelas AND siswa.kode_siswa='$_SESSION[kode]'");
                $ambildata = mysqli_fetch_array($qry);
            ?>
               <tr>
         ...
         <table class="responsive-table striped" cellspacing="0">
           ...
           <tbody>
             <?php
                $cari = mysqli_query($connect, "SELECT siswa.kode_siswa, siswa.nis, siswa.nama_siswa, nilai.semester, pelajaran.nama_pelajaran, nilai.nilai_tugas, nilai.nilai_tugas2, nilai.nilai_tugas3, nilai.nilai_uts, nilai.nilai_uas, nilai.keterangan, datakelas.jurusan, guru.nama_guru FROM siswa, nilai, pelajaran, datakelas, kelas, guru WHERE siswa.kode_siswa=nilai.kode_siswa AND nilai.kode_pelajaran=pelajaran.kode_pelajaran AND datakelas.kode_siswa=siswa.kode_siswa AND datakelas.kode_kelas=kelas.kode_kelas AND kelas.kode_kelas=nilai.kode_kelas AND kelas.kode_guru=guru.kode_guru AND kelas.kelas='$kelas' AND siswa.kode_siswa='$siswa'") or die(mysqli_error($connect));
                while ($cari2 = mysqli_fetch_array($cari)) {
                  ...
                  if ($nilai_rata2 >= 85 && $nilai_rata2 <= 100) {
                    $grade = "A";
                  } elseif ($nilai_rata2 >= 75 && $nilai_rata2 <= 84) {
                    $grade = "B";
                  } elseif ($nilai_rata2 >= 65 && $nilai_rata2 <= 74) {
                    $grade = "C";
                  } elseif ($nilai_rata2 >= 1 && $nilai_rata2 <= 64) {
                    $grade = "D";
              ...
              ?>
               <tr>
                 ...
                 <td>
                   <center><?php echo "$nilai_rata2"; ?></center>
                 </td>
                 <td>
                   <center><?php if ($grade == "A") {
                              echo $grade;
                            } else {
                              echo "-";
                            } ?></center>
