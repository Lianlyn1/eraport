...
if (isset($menu) and $aksi == "hapus") {
	...
	$tambah = mysqli_query($connect, "insert into guru(kode_guru, nip, username, password, nama_guru,   alamat, status, jenis_kelamin, foto,  telp)	VALUES('$_POST[kode_guru]','$_POST[nip]','$_POST[username]','$pass', '$_POST[nama_guru]',  '$_POST[alamat]', '$_POST[status]','$_POST[jenis_kelamin]', '$nama_baru' , '$_POST[telp]')");
	if ($tambah) {
		echo "<script>swal({
			title: 'Sukses!', 
	...
	if (mysqli_num_rows($cek) >= 1) {
		...
		$guru = explode(" | ", $_POST['kode_guru']);
		$kodeguru = $guru['1'];
		$edit = mysqli_query($connect, "Update kelas set tahun_ajar='$_POST[tahun_ajar]', kelas='$_POST[kelas]', nama_kelas='$_POST[nama_kelas]', status='$_POST[pilihstatus]', kode_guru='$kodeguru'
	...
	$kodesiswa = $siswa['1'];
	$cek = mysqli_query($connect, "SELECT * FROM datakelas where kode_kelas='$_POST[kode_kelas]' and kode_siswa='$kodesiswa'");
	if (mysqli_num_rows($cek) >= 1) {
		echo "<script>swal({
			title: 'Oops...', 
	...
	if (mysqli_num_rows($cek) >= 1) {
		...
		$edit = mysqli_query($connect, "Update datakelas set kode_kelas='$_POST[kode_kelas]', jurusan='$_POST[jurusan]', kode_siswa='$kodesiswa'
	...
	$passwordbaru2 = $_POST['password3'];
	$user = $_POST['username'];
	$tampil = mysqli_query($connect, "SELECT password from guru where username='$user'");
	$data = mysqli_fetch_array($tampil);
	if ($data['password'] == md5($passwordlama)) {
		$passwordbaruenkrip = md5($passwordbaru1);
		$edit = mysqli_query($connect, "Update guru set password='$passwordbaruenkrip'
								 where username='$user'") or die("gagal" . mysqli_error($connect));
		if ($edit) {
			echo "<script>swal({
			title: 'Sukses!', 
			text: 'Data telah terganti!', 
