<div class="row">
  <div class="col s12 m12 l12">
    <div class="card-panel">
      ...
      <div class="row">
        <?php
        $edit = mysqli_query($connect, "select * from nilai where kode_nilai='$_GET[id]'") or die("gagal" . mysqli_error($connect));
        $cek = mysqli_num_rows($edit);
        ...
        ?>
          <form action="aksi.php?menu=nilai&aksi=edit" method="post" onSubmit="return validasi_tambahnilai(this);" enctype="multipart/form-data" class="col s12">
            ...
            <div class="row">
              <div class="input-field col s12">
                <select name="semester" id="semester" disabled>
                  ...
                  <option value="1" <?php if ($data['semester'] == 1) {
                                      echo "selected ";
                                    } ?>>1 - Ganjil</option>
                  <option value="2" <?php if ($data['semester'] == 2) {
                                      echo "selected ";
                                    } ?>>2 - Genap</option>
                </select>
            ...
            <div class="row">
              <div class="input-field col s12">
                <select name="kode_pelajaran" id="kode_pelajaran" disabled>
                  ...
                  <?php
                  $sql = mysqli_query($connect, "SELECT * FROM pelajaran ORDER BY kode_pelajaran ASC");

                  while ($data2 = mysqli_fetch_array($sql)) {
                  ?>
                    <option value="<?php echo $data2['kode_pelajaran']; ?>" <?php if ($data2['kode_pelajaran'] == $data['kode_pelajaran']) {
                                                                              echo "selected";
                                                                            } ?>><?php echo $data2['nama_pelajaran']; ?></option>';
                  <?php
                  }
            ...
            <div class="row">
              <div class="input-field col s12">
                <?php
                $tampil = mysqli_query($connect, "SELECT * FROM guru, nilai where kode_nilai='$_GET[id]' AND guru.kode_guru=nilai.kode_guru ");
            ...
            <div class="row">
              <div class="input-field col s12">
                <select name="kode_kelas" id="kode_kelas" disabled>
                  ...
                  <?php
                  $sql = mysqli_query($connect, "SELECT * FROM kelas ORDER BY tahun_ajar, nama_kelas ASC");
                  if (mysqli_num_rows($sql) != 0) {
                    while ($data3 = mysqli_fetch_array($sql)) {
                  ?>
                      <option value="<?php echo $data3['kode_kelas']; ?>" <?php if ($data3['kode_kelas'] == $data['kode_kelas']) {
                                                                            echo "selected";
                                                                          } ?>><?php echo $data3['kelas']; ?> | <?php echo $data3['nama_kelas']; ?> | <?php echo $data3['tahun_ajar']; ?></option>';
            ...
            <div class="row">
              <div class="input-field col s12">
                <?php
                $tampil = mysqli_query($connect, "SELECT * FROM siswa, nilai where kode_nilai='$_GET[id]' AND siswa.kode_siswa=nilai.kode_siswa ");
                $tampildata2 = mysqli_fetch_array($tampil);
